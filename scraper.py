...
def show_entry_fields():
	...
	import csv
	download_dir = area_code + college_code.upper() + str(year) + branch.upper() + ".csv"
		...
		try:
			...
			for i in data:
				...
				if len(var1) == 7 or len(var1) == 8:
					if(str(var1)[4]) == "L" or str(var1[5]) == "L":
						if int(var5) >= 90:
							gvar = "S+"
							cvar = 2 * 10
							...
							gvar = "D"
							cvar = 2 * 5
						elif int(var5) >= 40 and int(var5) < 45:
							gvar = "E"
							cvar = 2 * 4
						else:
							gvar = "F"
